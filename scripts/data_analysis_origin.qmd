...
Plot_ACC <- Table_ACC %>% 
  ggplot(aes(x = factor(setsize), y = mean, group = condition, color = condition)) +
...
Plot_RT_benefits <- Table_RT_Benefits %>% 
  ggplot(aes(x = factor(setsize), y = mean, group = benefit, color = benefit)) +
  geom_line(position = dodge4)+
  geom_point(position = dodge4)+
  geom_errorbar(aes(ymin = mean - ci, ymax = mean + ci), width = .1, position = dodge4) +
  facet_wrap(~layout)+
...
Model_RT <-aov_4(
  formula = RT ~ condition * setsize * layout + (1 + condition * setsize * layout | participant), 
  data = mData_RT_condition)

emmeans(Model_RT, ~ condition | setsize * layout) %>% pairs()
...
Model_RT_positive <-aov_4(
  formula = PB ~ setsize * radius + (1 + setsize * radius | participant), 
  data = mData_RT_benefits)

emmeans(Model_RT_positive, ~ radius | setsize) %>% pairs()


Model_RT_negative <-aov_4(
  formula = NB ~ setsize * layout + (1 + setsize * layout | participant), 
  data = mData_RT_benefits)

emmeans(Model_RT_negative, ~ layout) %>% pairs()
