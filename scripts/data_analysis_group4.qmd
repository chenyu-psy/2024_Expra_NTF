...
mainPos_list <- Data_Res %>% 
  filter(ratio == "positive") %>% 
  select(participant)

mainNeg_list <- Data_Res %>% 
  filter(ratio == "negative") %>% 
  select(participant)

mainNeu_list <- Data_Res %>% 
  filter(ratio == "neutral") %>% 
  select(participant)
...
str_glue(
  "Mainly Positive: {n_distinct(mainPos_list$participant)} participants\n",
  "Mainly Negative: {n_distinct(mainNeg_list$participant)} participants\n",
  "Mainly Neutral : {n_distinct(mainNeu_list$participant)} participants"
) %>% message()
...
Plot_ACC <- Table_ACC %>% 
  ggplot(aes(x = ratio, y = mean, group = condition, color = condition)) +
...
Plot_RT_benefits <- Table_RT_Benefits %>% 
  ggplot(aes(x = ratio, y = mean, group = benefit, color = benefit)) +
  geom_line(position = dodge4)+
  geom_point(position = dodge4)+
  geom_errorbar(aes(ymin = mean - ci, ymax = mean + ci), width = .1, position = dodge4) +
  facet_wrap(~layout)+
